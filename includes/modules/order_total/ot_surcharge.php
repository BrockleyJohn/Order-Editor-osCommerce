...
*/
  ...
  class ot_surcharge {
    ...
    function process() {
     global $order, $currencies;
	  if (MODULE_PAYMENT_STATUS == 'true') {
         if ($GLOBALS['payment'] == MODULE_PAYMENT_TYPE) {
  ...
  function calculate_fee($amount) {
    ...
    $od_amount=0;
    $od_pc = $this->percentage;// + .04; //this is percentage plus the base fee
    $do = false;
...
// Calculate tax group deductions
	...
	if($od_amount < $this->min_fee) $od_amount = $this->min_fee;
    return $od_amount;
  }
...
// Check if gift voucher is in cart and adjust total
    ...
    for ($i=0; $i<sizeof($products); $i++) {
      ...
      $gv_query = tep_db_query("select products_price, products_tax_class_id, products_model from products where products_id = '" . $t_prid . "'");
    ...
    function keys() {
      return array('MODULE_PAYMENT_STATUS', 'MODULE_PAYMENT_SORT_ORDER','MODULE_PAYMENT_PERCENTAGE','MODULE_PAYMENT_MINIMUM', 'MODULE_PAYMENT_TYPE', 'MODULE_PAYMENT_INC_SHIPPING', 'MODULE_PAYMENT_INC_TAX','MODULE_MIN_FEE', 'MODULE_PAYMENT_CALC_TAX');
    }

    function install() {
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Mostrar Total', 'MODULE_PAYMENT_STATUS', 'true', 'Do you want to enable the Order Payment Fee?', '6', '1','tep_cfg_select_option(array(\'true\', \'false\'), ', now())");
      ...
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Minimum Fee', 'MODULE_MIN_FEE','0', 'Minimum amount fee.', '6', '2', now())");
